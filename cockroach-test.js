...
const newUser = async (uid_1, uname, artists, genres, basic_score) => {
...
const checkUserExists = async (uid_1) => {
...
const checkSlots = async (ref_code) => {
...
const addReferral = async (ref_code, uid) => {
  const user = await User.findOne({ where: { referral_code: ref_code } });
